...
object LocationsTable : Table("locations") {
    ...
    fun locationIdInDatabase(locationId: String) = transaction { select { LocationsTable.id eq locationId }.any() }
    ...
    fun saveLocation(location: Location, locationId: String? = null): String {
        ...
        transaction {
            if (locationId != null && locationIdInDatabase(locationId)) {
                update({ LocationsTable.id eq locationId }) {
                    locId = locationId;
                    writeLocation(it)
                }
            } else {
    ...
    fun deleteLocation(locationId: String) {
        transaction {
            deleteWhere { LocationsTable.id eq locationId }
        }
    }
    
    fun locationFromId(id: String): Location? {
        val resultRow = transaction { select { LocationsTable.id eq id }.firstOrNull() } ?: return null
