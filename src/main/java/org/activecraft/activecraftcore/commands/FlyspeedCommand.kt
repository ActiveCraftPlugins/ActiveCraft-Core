...
class FlyspeedCommand(plugin: ActiveCraftPlugin?) : ActiveCraftCommand("flyspeed", plugin!!) {
    ...
    public override fun runCommand(sender: CommandSender, command: Command, label: String, args: Array<String>) {
        ...
        if (level > 10 || level < -10) throw InvalidNumberException(args[0])
