...
import de.cplaiz.activecraftcore.commands.util.CommandUtils
...
import de.cplaiz.activecraftcore.exceptions.SelfTargetException
import de.cplaiz.activecraftcore.messagesv2.*
...
abstract class ActiveCraftCommandv2 @JvmOverloads constructor(
    ...
    protected fun isTargetSelf(
        ...
        if (sender.name.equals(target, ignoreCase = true)) {
            ...
            if (!ignorePermission && !sender.hasPermission(permString)) throw SelfTargetException(sender, "null")
            return true
        }
        return false
    }
    ...
    protected fun assertPermission(
        ...
        if (!permissible.hasPermission(permString)) throw NoPermissionException(permissible, permissionExtension)
        return true
    }
