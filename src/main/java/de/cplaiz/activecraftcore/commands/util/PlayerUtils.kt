...
import de.cplaiz.activecraftcore.exceptions.NoPlayerException
...
interface PlayerUtils {
    ...
    @Throws(InvalidPlayerException::class)
     fun getPlayer(input: String): Player? {
        if (Bukkit.getPlayer(input) == null) throw InvalidPlayerException(input)
    ...
    @Throws(NoPlayerException::class)
     fun getPlayer(sender: CommandSender): Player {
        if (sender !is Player) throw NoPlayerException(sender.name)
        return sender
    }

    @Throws(NoPlayerException::class)
    fun getOfflinePlayer(sender: CommandSender): OfflinePlayer {
        if (sender !is OfflinePlayer) throw NoPlayerException(sender.name)
        return sender
    }
    ...
    fun getOfflinePlayer(input: String): OfflinePlayer {
        ...
        try {
            ...
            if (http.responseCode != 200) throw InvalidPlayerException(input)
        } catch (e: IOException) {
    ...
    @Throws(InvalidPlayerException::class)
     fun getProfile(playername: String): Profilev2 {
        return Profilev2.of(playername) ?: throw InvalidPlayerException(playername)
    }
